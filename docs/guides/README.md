...
3. **Scan Types**
    ...
    - Available types: all, icmp, tcp, udp, arp
    - Example: `python src/main.py 192.168.1.1 -t tcp`

4. **Rate Limiting and UDP Ambiguity**
    - Use `--delay` to avoid flooding
    - Use `--udp-ambiguity open|closed` to classify no-response UDP outcomes

5. **Output File Safety**
    - `--append` appends output to existing files
    - `--force` allows overwrite/forced run in guarded paths
...
-   `open`: UDP response observed or ambiguity policy selected open
-   `closed`: ICMP unreachable observed or ambiguity policy selected closed
-   `error`: Error occurred while scanning
